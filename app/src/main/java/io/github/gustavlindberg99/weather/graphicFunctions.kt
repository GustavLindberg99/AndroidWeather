...
fun medianColor(context: Context, @DrawableRes image: Int): Int {
    ...
    val resolution =
        20    //Don't look at all pixels for performance reasons (this function is very slow otherwise, and no one will notice if the median isn't exact)
    val width: Int = bitmap.width
    val height: Int = bitmap.height
    val size: Int =
        ceil(width.toDouble() / resolution).toInt() * ceil(height.toDouble() / resolution).toInt()
