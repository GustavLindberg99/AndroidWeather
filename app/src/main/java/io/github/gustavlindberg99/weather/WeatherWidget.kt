...
abstract class WeatherWidget : Widget(R.layout.widget_weather, R.id.weather_widget, R.id.weather_widget_temperature, R.id.weather_widget_maxmin_temperatures, R.id.weather_widget_background, 0, 0){
    class Colored : WeatherWidget(){
        @ColorRes @DrawableRes override fun getBackground(@DrawableRes weatherBackground: Int): Int = if(weatherBackground == 0) R.color.skyBlue else weatherBackground
    }

    class Transparent : WeatherWidget(){
        @ColorRes override fun getBackground(@DrawableRes weatherBackground: Int): Int = R.color.transparent
    }

    class SemiTransparent : WeatherWidget() {
        @ColorRes override fun getBackground(@DrawableRes weatherBackground: Int): Int = R.color.semiTransparent
    }

    override fun onResize(appWidgetId: Int, newWidth: Int, newHeight: Int){
        val isSmall = newWidth < 300
        val isLarge = !isSmall && newHeight > 90
        this.setViewVisibility(appWidgetId, R.id.weather_widget_daily, if(isLarge) View.VISIBLE else View.GONE)
        this.setViewVisibility(appWidgetId, R.id.weather_widget_small_location_name, if(isSmall) View.VISIBLE else View.GONE)
        this.setViewVisibility(appWidgetId, R.id.weather_widget_details, if(isSmall) View.GONE else View.VISIBLE)
        this.setViewVisibility(appWidgetId, R.id.weather_widget_maxmin_temperatures, if(isSmall && newHeight < 75) View.GONE else View.VISIBLE)
        for(i in dayViews.indices){
            this.setViewVisibility(appWidgetId, dayViews[i], if(newWidth > 75 * (i + 1)) View.VISIBLE else View.GONE)
        }
    }
    ...
    override fun setDailyWeather(context: Context, views: RemoteViews, now: Calendar, maxTemperatures: List<Double>, minTemperatures: List<Double>, weatherCodes: List<Int>){
        ...
        for(i in 0..5){
            ...
            @DrawableRes val weatherImage = context.resources.getIdentifier("wmo_$weatherCode$dayOrNight", "drawable", context.packageName)
