...
class WeatherFragment : Fragment() {
    //Size 7
    private val _dailySummaries: List<TableRow>
        get() = listOf(
    ...
    private val _dayTemperatures: List<TextView>
        get() = this._dailySummaries.map {
            it.findViewById(
                R.id.temperature
            )
        }

    private val _hourlyWeatherBars: MutableList<LinearLayout?> =
        mutableListOf(null, null, null, null, null, null, null)
    private val _selectedHours: MutableList<Int> = mutableListOf(-1, -1, -1, -1, -1, -1, -1)
    private val _sunriseViews: MutableList<Button?> =
        mutableListOf(null, null, null, null, null, null, null)
    private val _sunsetViews: MutableList<Button?> =
        mutableListOf(null, null, null, null, null, null, null)
    private val _sunriseBorders: MutableList<View?> =
        mutableListOf(null, null, null, null, null, null, null)
    private val _sunsetBorders: MutableList<View?> =
        mutableListOf(null, null, null, null, null, null, null)
    ...
    private val _backgroundImage: ImageView
        get() = this.requireActivity().findViewById(R.id.background_image)
    private val _backgroundGradient: ImageView
        get() = this.requireActivity().findViewById(R.id.background_gradient)
    private val _backgroundColor: ImageView
        get() = this.requireActivity().findViewById(R.id.background_color)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        this._city = City.fromBundle(
            this.requireActivity(),
            this.arguments
        )!!    //This can only be null if the bundle that was sent is invalid, which shouldn't happen unless there is a bug
    }
        ...
        for (i in 0..6) {
            this._dayNames[i].setOnClickListener {
                ...
                if (hourlyWeatherBar != null) {
                    hourlyWeatherBar.visibility =
                        if (hourlyWeatherBar.visibility == View.GONE) View.VISIBLE else View.GONE
                }
    ...
    private fun createHourlyWeatherBar(day: Int) {    //Create these dynamically for performance reasons
        if (this._hourlyWeatherBars[day] == null || this._dayList.indexOfChild(this._hourlyWeatherBars[day]) == -1) {
            val hourlyWeatherBar: LinearLayout = View.inflate(
                this.requireActivity(),
                R.layout.hourly_weather_bar,
                null
            ) as LinearLayout
            hourlyWeatherBar.layoutParams = TableLayout.LayoutParams(
            ...
            for (i in 0..23) {
                this._hours[day].add(
                    hourlyWeatherBar.findViewById<TableRow>(R.id.daily_weather_bar)
                        .getChildAt(i) as Button
                )
                this._hourBorders[day].add(
                    hourlyWeatherBar.findViewById<TableRow>(R.id.hourly_borders).getChildAt(i)
                )
            }
    ...
    private fun refreshFromServer() {
        this._city.updateWeatherFromServer(
            { this.setLocation(this._city) },
            { errorMessage: String ->
    ...
    private fun updateWeather(data: WeatherData?) {
        ...
        //Current weather
        @StringRes val currentWeatherStringId = this.resources.getIdentifier(
            "wmo" + data.currentWeatherCode,
            "string",
            this.requireActivity().packageName
        )
        val currentWeatherString =
            if (currentWeatherStringId == 0) "--" else this.getString(currentWeatherStringId)
        val currentWindDirectionStringId = this.resources.getIdentifier(
            "wind" + (data.currentWindDirection / 22.5 + 1).roundToInt() % 16,
            "string",
            this.requireActivity().packageName
        )
        val currentWindString = if (data.currentWindSpeed == 0.0) Settings.UnitFormatter.windSpeed(
            this.requireActivity(),
            ...
            Locale.US,
            "%s %s",
            this.getString(currentWindDirectionStringId),
            Settings.UnitFormatter.windSpeed(this.requireActivity(), data.currentWindSpeed)
        )
        val firstSunrise: Calendar? = data.sunrises[0]
        val firstSunset: Calendar? = data.sunsets[0]
        this._currentSunrise.text = if (firstSunrise == null) "--"
        else Settings.UnitFormatter.time(
            this.requireActivity(),
            firstSunrise[Calendar.HOUR_OF_DAY],
            firstSunrise[Calendar.MINUTE]
        )
        this._currentSunset.text = if (firstSunset == null) "--"
        else Settings.UnitFormatter.time(
            this.requireActivity(),
            firstSunset[Calendar.HOUR_OF_DAY],
            firstSunset[Calendar.MINUTE]
        )
        this._currentTemperature.text =
            Settings.UnitFormatter.temperature(this.requireActivity(), data.currentTemperature)
        ...
        this._currentApparentTemperature.text = Settings.UnitFormatter.temperature(
            this.requireActivity(),
            data.currentApparentTemperature
        )
        this._currentPrecipitation.text =
            Settings.UnitFormatter.precipitation(this.requireActivity(), data.currentPrecipitation)
        this._currentPressure.text = Settings.UnitFormatter.pressure(data.currentPressure)
        this._currentUVIndex.text =
            Settings.UnitFormatter.uvIndex(this.requireActivity(), data.currentUvIndex)
        this._currentCloudCover.text = Settings.UnitFormatter.percentage(data.currentCloudCover)
        this._currentDewPoint.text =
            Settings.UnitFormatter.temperature(this.requireActivity(), data.currentDewPoint)
        this._currentPrecipitationProbability.text =
            Settings.UnitFormatter.percentage(data.currentPrecipitationProbability)
        this._currentAirQualityIndex.text = Settings.UnitFormatter.airQualityIndex(
            this.requireActivity(),
            data.currentAmericanAqi,
            data.currentEuropeanAqi
        )
        this._currentSeaTemperature.text = if (data.currentSeaTemperature.isNaN()) "--"
        else Settings.UnitFormatter.temperature(
            this.requireActivity(),
            data.currentSeaTemperature
        )
        ...
        if (this._city == (this.requireActivity() as MainActivity).selectedCity()) {
            ...
            if (backgroundImage == 0) {
                ...
                @ColorInt val medianColor = medianColor(
                    this.requireActivity(),
                    backgroundImage
                )    //Use the median instead of the average, otherwise the stars in the clear night background will cause the average color to be very bright
        ...
        for (day in 0..6) {
            ...
            for (hour in 0..23) {
                val hourOfDay: Int =
                    (if (day == 0) currentHour + 1 else 0) + hour    //Hour of day, can be greater than 24 if the view is for an hour tomorrow showing in the bar for today
                val i: Int =
                    day * 24 + hourOfDay     //Index of the current hour in the weekly length-168 arrays

                //Hours in weather bar
                this._hours[day][hour].text = String.format(
                    Locale.US,
                    "%s\n%s",
                    Settings.UnitFormatter.time(this.requireActivity(), hourOfDay),
                    Settings.UnitFormatter.temperature(
                        this.requireActivity(),
                        data.hourlyTemperature[i]
                    )
                )
                this._hours[day][hour].setCompoundDrawablesWithIntrinsicBounds(
                    0,
                    0,
                    0,
                    this.resources.getIdentifier(
                        "wmo_" + data.hourlyWeatherCode[i] + data.hourlyDayOrNight(i),
                        "drawable",
                ...
                //Hourly details bar
                val windDirectionStringId = this.resources.getIdentifier(
                    "wind" + (data.hourlyWindDirection[i] / 22.5 + 1).roundToInt() % 16,
                    "string",
                    requireActivity().packageName
                )
                val windSpeed = data.hourlyWindSpeed[i]
                val windString = if (windSpeed == 0.0) Settings.UnitFormatter.windSpeed(
                    requireActivity(),
                    windSpeed
                )
                else String.format(
                    Locale.US,
                    "%s %s",
                    this.getString(windDirectionStringId),
                    Settings.UnitFormatter.windSpeed(this.requireActivity(), windSpeed)
                )
                val precipitationProbability =
                    Settings.UnitFormatter.percentage(data.hourlyPrecipitationProbability[i])
                val uvIndex =
                    Settings.UnitFormatter.uvIndex(this.requireActivity(), data.hourlyUvIndex[i])
                val windView: TextView = hourlyDetailsBar.findViewById(R.id.wind)
                val precipitationProbabilityView: TextView =
                    hourlyDetailsBar.findViewById(R.id.precipitation_probability)
            ...
            val hourlyWeatherBarLayout: TableRow =
                hourlyWeatherBar.findViewById(R.id.daily_weather_bar)
            ...
            if (sunrise != null) {
                ...
                if (sunrise != null) {
                    val sunriseView = Button(
                        this.requireActivity(),
                        null,
                        androidx.appcompat.R.attr.buttonBarButtonStyle
                    )
                    ...
                    sunriseView.text = Settings.UnitFormatter.time(
                        this.requireActivity(),
                        sunrise[Calendar.HOUR_OF_DAY],
                        sunrise[Calendar.MINUTE]
                    ...
                    val sunriseBorder = View(this.requireActivity())
                    this._sunriseBorders[day] = sunriseBorder
                    sunriseBorder.setBackgroundColor(this.requireActivity().getColor(R.color.white))
                    sunriseBorder.layoutParams =
                        TableRow.LayoutParams(TableRow.LayoutParams.WRAP_CONTENT, dpToPx(0.5))
            ...
            if (sunset != null) {
                ...
                if (sunset != null) {
                    ...
                    sunsetView.text = Settings.UnitFormatter.time(
                        this.requireActivity(),
                        sunset[Calendar.HOUR_OF_DAY],
                        sunset[Calendar.MINUTE]
                    ) + "\n" + this.getString(R.string.sunset)
                    sunsetView.setCompoundDrawablesWithIntrinsicBounds(0, 0, 0, R.drawable.sunset)
                    val index: Int =
                        sunset[Calendar.HOUR_OF_DAY] + offset + 1 + if (sunrise != null && sunrise < sunset) 1 else 0
                    hourlyWeatherBarLayout.addView(sunsetView, index)

                    val sunsetBorder = View(this.requireActivity())
                    this._sunsetBorders[day] = sunsetBorder
                    sunsetBorder.setBackgroundColor(this.requireActivity().getColor(R.color.white))
                    sunsetBorder.layoutParams =
                        TableRow.LayoutParams(TableRow.LayoutParams.WRAP_CONTENT, dpToPx(0.5))
        ...
        for (i in 0..6) {
            ...
            this._dayTemperatures[i].text = String.format(
                Locale.US,
                "%s/%s",
                Settings.UnitFormatter.temperature(this.requireActivity(), data.minTemperature[i]),
                Settings.UnitFormatter.temperature(this.requireActivity(), data.maxTemperature[i])
            )
            this._dayWeathers[i].setImageResource(
                this.resources.getIdentifier(
                    "wmo_" + data.dailyWeatherCode[i] + dayOrNight,
                    "drawable",
                    this.requireActivity().packageName
                )
            )
            this._dayWeathers[i].contentDescription = this.getString(
                this.resources.getIdentifier(
                    "wmo" + data.dailyWeatherCode[i],
                    "string",
                    this.requireActivity().packageName
                )
            )
