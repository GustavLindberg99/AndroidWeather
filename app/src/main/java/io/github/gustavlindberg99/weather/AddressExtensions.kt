...
fun Address.cityName(context: Context): String {
    ...
    if(addressLine != null){
        val matcher = Pattern.compile(",[0-9\\s]*([^0-9,]+)[0-9\\s]*,\\s*" + Pattern.quote(this.countryName ?: "") + "$").matcher(addressLine)
        if(matcher.find()){
    ...
    return nullIfEmpty(this.subLocality) ?:
           nullIfEmpty(this.subAdminArea) ?:
           nullIfEmpty(this.adminArea) ?:
           nullIfEmpty(this.countryName, fixCapitalization=false) ?:
...
fun Address.timezone(context: Context): String {    //There are libraries that can do this, but they only check the coordinates which means they have to use a huge database which is slow and can easily run out of memory. This function is more efficient because since we already have the Address object we can find the timezone by looking at the country/region.
    ...
    val nonLocalizedAddress: Address by lazy(@Suppress("DEPRECATION") @Throws(IOException::class) fun(): Address {
        if(this.locale == Locale.US){
            return@lazy this
        }
        val results: List<Address> = Geocoder(context, Locale.US).getFromLocation(this.latitude, this.longitude, 1) ?: listOf()
        if(results.isEmpty()){
            return@lazy this
        }
        return@lazy results[0]
    })
    when(this.countryCode){
        ...
        "AR" -> return when(nonLocalizedAddress.adminArea?.replace("á", "a")?.replace("é", "e")?.replace("Provincia de ", "")?.replace(" Province", "")){
        ...
        "BR" -> {
            ...
            return when(nonLocalizedAddress.adminArea?.replace("á", "a")?.replace("ã", "a")?.replace("í", "i")?.replace("ô", "o")?.replace("State of ", "")){
        ...
        "CL" -> return if(this.longitude < -90) "Pacific/Easter"
                       else if((nonLocalizedAddress.adminArea ?: "").contains("Magallanes")) "America/Punta_Arenas"
        ...
        "KZ" -> return when(nonLocalizedAddress.adminArea?.replace(" Province", "")?.replace(" oblısı", "")?.replace("ı", "y")?.replace("ý", "y")) {
        ...
        "MX" -> return when(nonLocalizedAddress.adminArea?.replace("á", "a")?.replace("é", "e")?.replace("í", "i")?.replace("ó", "o")){
        ...
        "PG" -> return if((nonLocalizedAddress.adminArea ?: "").contains("Bougainville")) "Pacific/Bougainville" else "Pacific/Port_Moresby"
        ...
        "RU" -> {
            when(nonLocalizedAddress.adminArea?.replace("'", "")?.replace("á", "a")?.replace("ó", "o")?.replace("ú", "u")?.replace("Respublika", "")?.replace("Republic of ", "")?.replace("Republic ", "")?.replace("Oblast", "")?.replace("oblast", "")?.replace(" Autonomous Okrug", "")?.replace(" avtonomnyy okrug", "")?.replace("Krai", "")?.replace("kray", "")?.replace("skaya", "")?.replace("skaja", "")?.trim()){
